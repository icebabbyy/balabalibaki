...
const ORDER_STATUS_URL = process.env.ORDER_STATUS_URL || "https://wishyoulucky.page/order-status";
...
const paymentChannelText = (m) => (m === "truemoney" ? "TrueMoney Wallet" : "โอนผ่านธนาคาร (K SHOP QR)");
...
<tr>
  <td style="padding:8px 0;">
    <div style="display:flex;gap:10px;align-items:center;">
      ${it.image ? `<img src="${it.image}" width="56" height="56" style="border-radius:8px;object-fit:cover;border:1px solid #eee;" />` : ""}
      <div>
  ...
  <td style="text-align:right;font-weight:600;">${it.price ? thb(Number(it.price) * Number(it.quantity)) : "-"}</td>
</tr>`
    )
    .join("");
...
const htmlTemplate = (p) => {
  ...
  const link = `${ORDER_STATUS_URL}?order=${encodeURIComponent(p.order_number)}&email=${encodeURIComponent(p.to)}`;
  ...
  <div style="border:1px solid #eee;border-radius:12px;padding:16px;margin-top:12px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin:8px 0;">
      ...
      <tbody>
        ...
        <tr>
          <td style="padding-top:8px;border-top:1px dashed #ddd;">รวมทั้งสิ้น</td>
          <td style="text-align:right;padding-top:8px;border-top:1px dashed #ddd;font-weight:700;">${thb(p.total_price)}</td>
        </tr>
      </tbody>
    </table>

    <p style="margin:12px 0 4px;"><strong>รูปแบบการชำระเงิน:</strong> ${paymentTypeText(p.total_price, p.deposit)}</p>
    <p style="margin:4px 0;"><strong>ยอดชำระแล้ว:</strong> ${thb(p.paid_amount)}</p>
    <p style="margin:4px 0;"><strong>ยอดที่เหลือ:</strong> ${thb(balance)}</p>
    <p style="margin:4px 0 12px;"><strong>ช่องทางการชำระเงิน:</strong> ${paymentChannelText(p.payment_method)}</p>
    ...
    <div style="padding:12px;background:#faf5ff;border:1px solid #eee;border-radius:10px;margin-top:8px;">
      ...
      ${p.customer.note ? `<div style="margin-top:6px;"><strong>หมายเหตุจากลูกค้า:</strong> ${p.customer.note}</div>` : ""}
    </div>
...
ยอดชำระแล้ว: ${thb(p.paid_amount)}  คงเหลือ: ${thb(Math.max(Number(p.total_price) - Number(p.paid_amount || 0), 0))}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (sent?.error) {
      ...
      return { statusCode: 500, headers: cors(event.headers?.origin), body: JSON.stringify({ ok: false }) };
    }

    return { statusCode: 200, headers: cors(event.headers?.origin), body: JSON.stringify({ ok: true }) };
  } catch (e) {
    console.error(e);
    return { statusCode: 500, headers: cors(event.headers?.origin), body: JSON.stringify({ error: e?.message || "Internal Error" }) };
  }
};
