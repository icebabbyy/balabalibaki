...
import { ShoppingCart, User, Heart, Package, LogOut, Search, FileQuestion, Truck, Undo2, ArrowRight, ChevronDown } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import { useCart } from "@/hooks/useCart";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { toDisplaySrc } from "@/lib/imageUrl"; // ✅ ใช้ตัวเดียว
    ...
    <li>
      <NavigationMenuLink asChild>
        ...
        >
          {image && (
            <img
              src={toDisplaySrc(image, { w: 80, q: 85 })}
              alt={title || ""}
...
const Header = () => {
  ...
  const navLinkStyles =
    "group inline-flex h-10 w-max items-center justify-center rounded-md bg-transparent px-4 py-2 text-sm font-medium transition-colors text-white hover:bg-white/20 focus:bg-white/20 focus:outline-none";
...
{/* === หมวดหมู่สินค้า: คลิกข้อความ = ไป /categories, คลิกลูกศร = เปิดเมนู === */}
<NavigationMenuItem>
  {/* ใช้ Trigger เดียวเพื่อให้ hover แล้วเมนูเปิดได้ตามปกติ */}
  <NavigationMenuTrigger
  className={cn(navTriggerClass, "[&>svg]:hidden")} // ซ่อน chevron ที่ติดมากับ Trigger
  onClick={(e) => {
    const target = e.target as HTMLElement;
    const onCaret = target.closest("[data-caret]");
    if (onCaret) return;          // คลิก caret = แค่ toggle เมนู
    e.preventDefault();           // คลิกข้อความ = ไปหน้าหมวดหมู่
    navigate("/categories");
  }}
>
  <span className="mr-1">หมวดหมู่สินค้า</span>
  {/* อันนี้คือ caret ของเราเอง (คงไว้หนึ่งอัน) */}
  <span data-caret className="ml-1 inline-flex items-center">
  ...
  <NavigationMenuContent>
    <ul className="grid w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px]">
      ...
      {/* ปุ่มล่าง "ดูหมวดหมู่ทั้งหมด" */}
      <li className="col-span-1 md:col-span-2">
        <NavigationMenuLink asChild>
          <Link
            to="/categories"
            className="flex h-full w-full select-none flex-col items-center justify-center rounded-md p-4 text-white no-underline outline-none transition-transform hover:scale-105"
            style={{ backgroundColor: "#a475c9" }}
          >
            <div className="text-base font-bold">
              ดูหมวดหมู่ทั้งหมด <ChevronDown className="inline-block h-4 w-4 -rotate-90" />
            </div>
          </Link>
        </NavigationMenuLink>
      </li>
          ...
          <div className="flex items-center justify-end space-x-2">
            <form onSubmit={handleSearch} className="hidden lg:flex items-center">
              <div className="relative">
                ...
                <Button
                  type="submit"
                  size="icon"
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 h-7 w-7 rounded-full text-white hover:bg-purple-700"
                  style={{ backgroundColor: "#a475c9" }}
                >
            ...
            {user ? (
              ...
              <Button
                ...
                size="sm"
                className="hidden md:inline-flex text-white hover:bg-purple-700"
                style={{ backgroundColor: "#a475c9" }}
              >
                เข้าสู่ระบบ
              </Button>
