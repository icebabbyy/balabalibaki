...
const Index = () => {
  ...
  const fetchAndMapProducts = async (queryBuilder: any): Promise<ProductPublic[]> => {
  ...
  const fetchFeaturedProducts = async () => {
    try {
      const products = await fetchAndMapProducts(
        supabase.from('products').select('*, product_images(image_url, "order")').limit(8).order('created_at', { ascending: false })
      );
  ...
  const fetchRandomHomepageProducts = async (allCategories: Category[]) => {
    try {
      ...
      for (const category of randomCategories) {
        const products = await fetchAndMapProducts(
          supabase.from('products').select('*, product_images(image_url, "order")').eq('category', category.name).limit(5)
        );
