...
  const [wishlist, setWishlist] = useState<Set<number>>(new Set()); // state ใหม่สำหรับเก็บ ID สินค้าใน wishlist

  const [wishlistLoading, setWishlistLoading] = useState(false);

  useEffect(() => {

    // ฟังก์ชันสำหรับดึง wishlist ของ user ที่ login อยู่

    const fetchWishlist = async () => {

      if (!user) {

        setWishlist(new Set()); // ถ้า user logout ให้เคลียร์ wishlist
...
  }, [user]); // ให้ดึงใหม่ทุกครั้งที่สถานะ user เปลี่ยน (login/logout)
...
        // อัปเดต State โดยการเพิ่ม ID เข้าไป
