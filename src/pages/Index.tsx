...
interface Banner { id: number; image_url: string; }
...
const Index = () => {
  ...
  const fetchBanners = async () => { /* ...โค้ดเดิม ไม่ต้องแก้ไข... */ };

  const fetchFeaturedProducts = async () => {
    try {
      const { data, error } = await supabase.from('public_products').select('*').limit(10).order('created_at', { ascending: false });
      if (error) throw error;
      setFeaturedProducts((data || []).map(transformProductData));
    } catch (error) { console.error('Error fetching featured products:', error); }
  };
  
  const fetchHomepageCategories = async () => { /* ...โค้ดเดิม ไม่ต้องแก้ไข... */ };
  ...
  const handleToggleWishlist = async (product: ProductPublic) => {
    ...
    try {
      if (isCurrentlyInWishlist) {
        ...
        setWishlist(prev => { const newWishlist = new Set(prev); newWishlist.delete(product.id); return newWishlist; });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {products.map((product: ProductPublic) => (
              <EnhancedProductCard 
                ...
                isWishlistLoading={wishlistLoading && product.id === wishlistLoading} // Optional: loading effect per card
              />
            ))}
          </div>
